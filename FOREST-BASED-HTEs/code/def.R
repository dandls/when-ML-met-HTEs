...
study_name <- "cftest_repl100" # must be different from the names in results
...
methods <- c("cf")  # c("cf","cfhonest", "mob","mobhonest", "hybrid", "hybridhonest", "equalized", "equalizedhonest", "mobcf", "mobcfhonest")

# only run subset of studies? --> provide study IDs (based on DGP.R)
StudyIDs <- c(1,3) # e.g. c(1, 2)
